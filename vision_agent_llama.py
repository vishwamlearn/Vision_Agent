...
# Hugging Face imports for the NVIDIA model
...
# --- Load the NVIDIA Llama-3.1-Nemotron-Nano-VL-8B-V1 model globally ---
...
try:
    ...
    print(f"Failed to load NVIDIA model: {e}")
...
def llama_analysis_node(state: AgentState):
    """Node to have the Llama model analyze the annotated image and provide feedback."""
    if llama_model is None or llama_processor is None:
        raise RuntimeError("NVIDIA Llama model not loaded. Please ensure you have the correct setup.")
    ...
    # The Llama model requires a specific chat format with the image
    messages = [
        {"role": "user", "content": [
    ...
    input_tensors = llama_processor.apply_chat_template(
        messages, 
    ...
    response_tokens = llama_model.generate(**input_tensors, max_new_tokens=100)
    response_text = llama_processor.decode(response_tokens[0], skip_special_tokens=True)
    
    # The Llama chat template adds a turn. We need to parse out the model's response.
    cleaned_response = response_text.split("model\n")[1].strip() if "model\n" in response_text else response_text

    print(f"Llama response: {cleaned_response}")
...
workflow.add_node("llama_analysis", llama_analysis_node)

workflow.add_edge(START, "run_grounding_dino")
workflow.add_edge("run_grounding_dino", "llama_analysis")
workflow.add_conditional_edges("llama_analysis", decide_to_continue, {"run_grounding_dino": "run_grounding_dino", "end": END})
...
iface = gr.Interface(
    ...
    title="LangGraph-powered Image Detection Agent with Llama Nemotron Nano VL",
    description="Upload an image and specify what to detect. The agent will use Llama-3.1-Nemotron-Nano-VL-8B-V1 to refine its search until the detection is satisfactory."
)
